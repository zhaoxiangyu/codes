...
public class ClassUi2 {
	
	public interface EventListener {
		void event(ClassUi2 cu);
	}
	...
	public JPanel getMethodTable(Class clazz, int type){
		...
		for (Iterator iterator = sms.iterator(); iterator.hasNext();) {
			...
			panel.add(new JLabel(m.getName()),gbc);
			...
			for (int j = 0; j < pars.length; j++) {
				gbc = new GridBagConstraints(1+2+(2*j),i,1,1,
						0.001,i/3.0,
						GridBagConstraints.FIRST_LINE_START,
						GridBagConstraints.NONE,
						new Insets(2, 2, 2, 2),0,0);
				panel.add(new JLabel(pars[j].getSimpleName()),gbc);			
				gbc = new GridBagConstraints(1+2+(2*j+1),i,1,1,
						0.001,i/3.0,
						GridBagConstraints.FIRST_LINE_START,
						GridBagConstraints.NONE,
						new Insets(2, 2, 2, 2),0,0);
				panel.add(new JTextField(8),gbc);			
			}
			gbc = new GridBagConstraints(1+2+2*pars.length,i,1,1,
					0.001,i/3.0,
					GridBagConstraints.FIRST_LINE_START,
					GridBagConstraints.NONE,
					new Insets(2, 2, 2, 2),0,0);
			panel.add(new JLabel(")"),gbc);
			gbc = new GridBagConstraints(1+2+2*pars.length+1,i,1+(2*(maxarglen-pars.length)),1,
					0.999,i/3.0,
					GridBagConstraints.FIRST_LINE_START,
					GridBagConstraints.NONE,
					new Insets(2, 2, 2, 2),0,0);
