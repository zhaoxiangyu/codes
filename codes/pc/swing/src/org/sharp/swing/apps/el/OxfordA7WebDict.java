...
import static org.sharp.jdkex.LangUtils.affix;
import static org.sharp.jdkex.LangUtils.join;
...
public class OxfordA7WebDict implements WebDict {
	...
	public static class Parser {
		...
		private String[] parseXGs(Element ng) {	//example sentence
			...
			return Utils.toArray(examples, String.class);
		}
		...
		private String[] parseXRs(Element xrg) {	//synonym
			...
			return Utils.toArray(ret, String.class);
		}
		...
		private String parseOptValue(Element ele,String cla){
			...
			if(el!=null){
				...
				log("<"+cla+">"+value);
				return Utils.defaultIfNull(value,"");
			}
			return "";
		}
		...
		private String[] parseChildValues(Element ele,String cla){
			...
			return Utils.toArray(strs,String.class);
		}
		...
		private String parseValue(Element ele,String cla, String def){
			...
			if(el!=null){
				String value = el.getTextExtractor().toString();
				log("<"+cla+">"+value);
				return Utils.defaultIfNull(value,def);
			}
			return def;
		}
		...
		protected void parseHG(Element entry){
			...
			if(bri_pro!=null){
				pos.setUkPro(bri_pro.getTextExtractor().toString());
				Element i_pro = bri_pro.getFirstElementByClass("sound");
				if(i_pro!=null){
					String oc = i_pro.getAttributeValue("onclick");
					System.out.println("un_pro mp3 url:"+LangUtils.subStringBtw(oc,"'","'"));
				}
			}
			
			Element ame_pro =  hg.getContent().getFirstElementByClass("y");
			if(ame_pro!=null){
				pos.setUsPro(ame_pro.getTextExtractor().toString());
				Element y_pro =	ame_pro.getFirstElementByClass("sound");
				if(y_pro!=null){
					String oc = y_pro.getAttributeValue("onclick");
					System.out.println("us_pro mp3 url:"+LangUtils.subStringBtw(oc,"'","'"));
				}
			}
	...
	public Term lookup(String word) {
		...
		if(poses == null)
			return null;
		for (String href : poses.values()) {
			PartOfSpeech pos = lookupPOS(href);
			if(pos!=null)
				t.setPses(LangUtils.add(t.getPses(), pos));
		}
		return t;
	}
	
	private PartOfSpeech lookupPOS(String posUrl){
		Reader input = fetchHtml(posUrl);
	...
	public static void saveWordTo(String word,String filePath){
		Reader r = fetchHtml(word);
		String conent;
		try {
			conent = IOUtils.toString(r);
			FileUtils.writeStringToFile(new File(filePath), conent);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
