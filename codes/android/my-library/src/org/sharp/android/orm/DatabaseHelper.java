...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase arg0) {
		create(mContext, arg0);
	}
	
	public static void create(Context ctx, SQLiteDatabase arg0){

		ArrayList<Class<?>> tables = getEntityClasses(ctx);

		Log.i(LogParams.LOGGING_TAG, "Creating " + tables.size() + " tables");
		List<String> primarys = new ArrayList<String>();
		StringBuffer sb;
		for (Class<?> table : tables) {
			List<ColumnAttribute> columns = Util.getTableColumn(((HadiApplication)ctx.getApplicationContext()),table);
			if(columns.size()==0){
				continue;
			}
			sb = new StringBuffer();
	...
	public static void drop(Context ctx, SQLiteDatabase arg0){
		ArrayList<Class<?>> tables = getEntityClasses(ctx);
		for (Class<?> table : tables) {
	...
	private static ArrayList<Class<?>> getEntityClasses(Context context) {
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				try {
					Log.d("ActiveORM", "name:"+name);
					if(/*!"org.apache.commons.lang.SystemUtils".equals(name) ||
							!"org.apache.commons.lang.builder.ToStringBuilder".equals(name)*/
							!name.startsWith("org.apache") &&
							!"org.sharp.vocreader.core.HandwritingSupport".equals(name))
