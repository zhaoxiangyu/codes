...
public class DefaultDAO {
	...
	public DefaultDAO(Context context,String dbFilePath){
		this.mApplication = ((HadiApplication)context.getApplicationContext());
		this.mDatabaseManager = new DatabaseManager(context, dbFilePath);
		this.mContext = context;
	}
	...
	public static class SqlClause{
		...
		public SqlClause(String whereClause, String[] whereArgs){
			this.whereClause = whereClause;
			this.whereArgs = whereArgs;
		}
	}
	
	private SqlClause primaryKey2Clause(Object model){
		List<ColumnAttribute> atas = Util.getTableColumn(mApplication, (Class<?>)model.getClass());
		String whereClause = "";
		List<String> whereArgs = new ArrayList<String>();
		for(ColumnAttribute ata:atas){
		...
		return new SqlClause(whereClause.substring(4),
	...
	public List<Object> select_by_primary(Object model){
		SQLiteDatabase db = this.mDatabaseManager.open();
		SqlClause sc = primaryKey2Clause(model);
		Cursor cursor = db.query(true, 
				Util.getTableName(model.getClass()),
				null, 
				sc.whereClause, 
				sc.whereArgs, null, null, null, null);
		List<Object> list = Util.processCursor(mContext,
				model.getClass(),
				cursor);
		cursor.close();
		db.close();
